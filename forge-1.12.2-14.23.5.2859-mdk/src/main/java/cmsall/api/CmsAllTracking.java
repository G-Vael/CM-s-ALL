 ...
 *     CmsAllTracking.relocate(world, landedFromTo); // only blocks that landed; map each source BlockPos to its destination
 * } finally {
...
public final class CmsAllTracking {
    ...
    public static int relocate(World world, Map<BlockPos, BlockPos> fromTo) {
        return PlacedBlocksTracker.relocate(world, fromTo);
    }
