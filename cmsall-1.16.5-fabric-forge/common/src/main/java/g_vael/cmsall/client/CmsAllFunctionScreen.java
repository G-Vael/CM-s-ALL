...
public class CmsAllFunctionScreen extends Screen {
    ...
    public CmsAllFunctionScreen(Screen parent, ServerConfig working, Functions.Kind kind, boolean canEdit) {
        super(new TranslatableComponent(titleKey(kind)));
        this.parent = parent;
    ...
    protected void init() {
        ...
        Button blocks = new Button(x, 50, 96, 20, new TranslatableComponent("cmsall.gui.list.blocks"), b -> {
            serverList = 0;
            listScroll = 0;
            reinit();
        });
        ...
        Button tools = new Button(x + 100, 50, 96, 20, new TranslatableComponent("cmsall.gui.list.tools"), b -> {
            serverList = 1;
            listScroll = 0;
            reinit();
        });
        ...
        Button add = new Button(x + panelW() - 96, 50, 96, 20, new TranslatableComponent("cmsall.gui.add"),
                b -> minecraft.setScreen(new CmsAllAddScreen(this, toolMode(), currentList(), kind, this::pushEdit)));
        add.active = canEdit;
        addButton(add);

        EditBox searchBox = new EditBox(this.font, x, 74, panelW(), 18, new TranslatableComponent("cmsall.gui.search"));
