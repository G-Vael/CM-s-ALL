...
public final class ConfigManager {
    ...
    private static void applyInternal() {
        ...
        RuntimeConfig.allowedModes = server.allowedModes == null
                ? Collections.singleton("hold") : new HashSet<>(server.allowedModes);
    ...
    private static Set<Block> resolveBlocks(Iterable<String> ids) {
        ...
        for (String id : ids) {
            ...
            if (rl != null) {
                Registry.BLOCK.getOptional(rl).ifPresent(out::add);
            }
        }
        return out;
    }
    ...
    private static Set<Item> resolveItems(Iterable<String> ids) {
        ...
        for (String id : ids) {
            ResourceLocation rl = ResourceLocation.tryParse(id);
            if (rl != null) {
                Registry.ITEM.getOptional(rl).ifPresent(out::add);
            }
        }
        return out;
    }
