...
/** Vertical scroller for a fixed set of widgets: register each with its logical Y, then relayout() offsets+hides them to fit a viewport. No-op (no scroll) when everything fits. */
public final class ScrollPane {
    ...
    public void renderBar(net.minecraft.client.gui.GuiGraphics g, int panelRight) {
        if (!scrollable()) {
            return;
        }
        ...
        g.fill(x, thumbY, x + 2, thumbY + thumbH, 0xA0FFFFFF);
    }
}
