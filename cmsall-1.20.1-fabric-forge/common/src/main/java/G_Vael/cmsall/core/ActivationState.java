...
public final class ActivationState {
    ...
    private record State(ActivationMode mode, boolean toggledOn) {
    }
    ...
    public static boolean isActive(ServerPlayer player) {
        return switch (mode(player)) {
            case HOLD -> player.isShiftKeyDown();
            case SNEAK_INVERT -> !player.isShiftKeyDown();
            case ALWAYS -> true;
            case TOGGLE -> toggledOn(player);
        };
    }
}
