...
public final class CmsAllCommand {
    ...
    private static com.mojang.brigadier.builder.RequiredArgumentBuilder<CommandSourceStack, Integer> trackMaxArg(Functions.Kind kind) {
        return Commands.argument("max", com.mojang.brigadier.arguments.IntegerArgumentType.integer(1, 131072))
                .executes(ctx -> setTrackMax(ctx, kind, com.mojang.brigadier.arguments.IntegerArgumentType.getInteger(ctx, "max")));
    }
    ...
    private static List<String> listFor(Functions.Kind kind, boolean tool) {
        ...
        return switch (kind) {
            case MINE -> tool ? c.mineTools : c.mineBlocks;
            case CUT -> tool ? c.cutTools : c.cutBlocks;
            case DIG -> tool ? c.digTools : c.digBlocks;
        };
    }
    ...
    private static int setMode(CommandContext<CommandSourceStack> ctx, String raw) throws CommandSyntaxException {
        ...
        if (!RuntimeConfig.allowedModes.contains(key)) {
            player.displayClientMessage(Component.translatable("cmsall.msg.mode_denied", key), true);
            return 0;
        }
        ActivationMode mode = ActivationMode.parse(key, RuntimeConfig.defaultMode);
        ActivationState.setMode(player, mode);
        player.displayClientMessage(
                Component.translatable("cmsall.msg.mode", Component.translatable("cmsall.mode." + mode.id())), true);
        return 1;
    }
    ...
    private static int toggle(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (ActivationState.mode(player) != ActivationMode.TOGGLE) {
            player.displayClientMessage(Component.translatable("cmsall.msg.toggle_denied"), true);
            return 0;
        }
        boolean on = ActivationState.flipToggle(player);
        player.displayClientMessage(
                Component.translatable("cmsall.msg.toggle", Component.translatable(on ? "cmsall.gui.on" : "cmsall.gui.off")), true);
        return 1;
    }
    ...
    private static int setReplant(CommandContext<CommandSourceStack> ctx, Boolean value) throws CommandSyntaxException {
        ...
        player.displayClientMessage(
                Component.translatable("cmsall.msg.replant", Component.translatable(on ? "cmsall.gui.on" : "cmsall.gui.off")), true);
        return 1;
    }
    ...
    private static int setProtect(CommandContext<CommandSourceStack> ctx, Boolean value) throws CommandSyntaxException {
        ...
        player.displayClientMessage(
                Component.translatable("cmsall.msg.protect", Component.translatable(on ? "cmsall.gui.on" : "cmsall.gui.off")), true);
        return 1;
    }
    ...
    private static void setTrackEnabled(Functions.Kind kind, boolean value) {
        ServerConfig c = ConfigManager.server();
        switch (kind) {
            case MINE -> c.trackMine = value;
            case CUT -> c.trackCut = value;
            case DIG -> c.trackDig = value;
        }
    }
    ...
    private static int setTrackMax(CommandContext<CommandSourceStack> ctx, Functions.Kind kind, int max) {
        ...
        ServerConfig c = ConfigManager.server();
        switch (kind) {
            case MINE -> c.trackMineMax = clamped;
    ...
    private static int resetTrack(CommandContext<CommandSourceStack> ctx, Functions.Kind kindOrNull) {
        ...
        ctx.getSource().sendSuccess(() -> Component.translatable("cmsall.msg.track_reset", name), true);
        return 1;
    }
    ...
    private static void feedback(CommandContext<CommandSourceStack> ctx, String message) {
        ctx.getSource().sendSuccess(() -> Component.literal("[CM'sALL] " + message), true);
    }
}
