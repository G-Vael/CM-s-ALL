...
public final class PlacedBlocksData extends SavedData {
    ...
    public static PlacedBlocksData load(CompoundTag tag, HolderLookup.Provider provider) {
        ...
        for (int i = 0; i < KEYS.length; i++) {
            ...
            for (int j = 0; j < positions.length; j++) {
                data.sets[i].put(positions[j], hasKeys ? keys[j] : 0); // old saves migrate lazily via sentinel 0
            }
        }
        return data;
    }
    ...
    /** the factory is stateless — share one instance instead of allocating per tracker access. */
    private static final SavedData.Factory<PlacedBlocksData> FACTORY =
            new SavedData.Factory<>(PlacedBlocksData::create, PlacedBlocksData::load, null);
